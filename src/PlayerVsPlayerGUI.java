...
public class PlayerVsPlayerGUI implements EventHandler<ActionEvent> {
	...
	public PlayerVsPlayerGUI(Scene previousScene, Stage primaryStage) {
	...
	public void handle(ActionEvent action) {
		...
		Label instructions = new Label("Please enter the names for both Players (not required):");
		...
		Label lbName2 = new Label("Player 2 Name:");
		TextField tfName2 = new TextField();
		tfName2.setPromptText("Name");
		
		Button btSubmit = new Button("Submit");
		
		infoContainer.add(lbName1, 0, 0);
		infoContainer.add(tfName1, 1, 0);
		infoContainer.add(lbName2, 0, 1);
		infoContainer.add(tfName2, 1, 1);
		infoContainer.add(btSubmit, 0, 2);
		...
		primaryStage.setTitle("Connect Four - Player Vs. Player");
		...
		btSubmit.setOnAction(e -> {
			...
			this.setPlayerName(tfName1.getText(), tfName2.getText());
			...
			// Alternative for constructor event handler:
			...
			gameBoard.getChildren().forEach(item -> {
				...
				item.setOnMouseClicked(f -> {
					...
					if(!currentDisc.isEmpty() && (ConnectFourBoard.getRowIndex(currentDisc) == 5)) {
						...
						if(ConnectFourDisc.getTurnID() % 2 == 0) {
							
							lbCurrentPlayer.setText(player2Name);
							
							ConnectFourDisc.incrementTurnID();
							
							currentPlayerDisc.setFill(ConnectFourDisc.getCurrentColor());
							discSample.setFill(ConnectFourDisc.getCurrentColor());
							...
							} else if(gameBoard.gradeYellow()) {
								
								Alert alert = new Alert(AlertType.INFORMATION);
								alert.setTitle("ConnectFour - Victory!");
								alert.setHeaderText(null);
								alert.setContentText(player2Name + " has won!");
								alert.showAndWait();
								player1Name = "Player 1";
								player2Name = "Player 2";
						...
						if(ConnectFourDisc.getTurnID() % 2 == 0) {
							...
							gameBoard.addYellowCoordinates(item);
